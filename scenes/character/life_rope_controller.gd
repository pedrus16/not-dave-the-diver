...
func create() -> void:
	...
	rope.anchors = [_surface_anchor, _character_anchor]
...
func _closest_point(from: Vector2) -> Dictionary:
	var points := rope.data.get_global_points()
...
func _process_retractation(delta: float) -> void:
	...
	if _retract_timer < 1.0 / retract_speed || rope.data.get_count() <= 2:
		return
